...
Kuş verisi olarak, Türkiye ve dünyadaki en kapsamlı kuş gözlem veri tabanı olan [eBird](https://ebird.org)'ü kullanacağız. Kuş gözlemcileri araziye çıktıklarında gözlemledikleri kuşları bu veri tabanına kaydediyor, bilime ve doğa korumaya katkı sağlayabiliyorlar. 2015 yılında ulusal veri tabanımız Kuşbank'ın eBird'e taşınmasından beri Türkiye kuşlarını konu alan en kapsamlı veri tabanı aynı zamanda.
...
İndirdiğimiz eBird verilerini R'a yükleyelim. eBird verileri txt dosyası olduğundan [{tidyverse}](https://www.tidyverse.org/) paket evreni içindeki bir fonksiyon olan [read_delim()](https://readr.tidyverse.org/reference/read_delim.html)'i kullandık. Base R'daki [read.table()](https://www.rdocumentation.org/packages/utils/versions/3.6.2/topics/read.table) fonksiyonu da kullanılabilir. Veri biraz büyük olduğundan yüklenmesi ve işlenmesi yavaş olabilir.
...
[print()](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/print) fonksiyonu ile verinin genel yapısını gördük. 2,403,720 gözlem (satır) ve 50 değişkene (sütun) sahip bir tibble ({tidyverse}'e özel bir data frame şekli, bence daha kullanışlı). Değişken sayısı fazla olduğu için print() fonksiyonu yeterince kullanışlı. Bu sebeple, R'a yüklediğimiz eBird veri tablosunun tüm sütunlarını ve onların yapılarını görmek için [glimpse()](https://dplyr.tidyverse.org/reference/glimpse.html) fonksiyonunu kullanacağız. Bu fonksiyon, base R'daki [str()](https://www.rdocumentation.org/packages/utils/versions/3.6.2/topics/str) fonksiyonuna benziyor ancak tibble veri yapısıyla kullanılırken daha sade ve kullanışlı bence. Özetle, bu fonksiyonu, print() fonksiyonunun transpoze edilmiş hâli olarak görebilirsiniz.
...
Şimdi de [{tmap}](https://r-tmap.github.io/tmap/) paketinin içindeki [qtm()](https://r-tmap.github.io/tmap/reference/qtm.html) fonksiyonu ile çizelim.

```{r}
qtm(tr_il)
```

Daha az yazarak gayet güzel bir sonuç elde ettik. Yukarıda da gördüğünüz gibi harita çizmek için farklı yöntemler mevcut. Ben genelde [{ggplot2}](https://ggplot2.tidyverse.org/) ve {tmap} paketlerini tercih ediyorum. Tercih size kalmış.
...
![Album of Abyssinian birds and mammals | Chicago :Field Museum of Natural History,1930. | [Kaynak](https://biodiversitylibrary.org/page/2836721)](figs/bearded.jpg)
...
![Coloured figures of the birds of the British Islands. v.1 | London :R. H. Porter,1885-1897. | [Kaynak](https://biodiversitylibrary.org/page/34514933)](figs/griffon.jpg)
